            ...
            <div class="group" v-for="article of articles" :key="article">
                <nuxt-link :to='`article/${article.slug}`'>
                  ...
                  <div class="px-0 md:px-4 md:pl-9">
                      <p class="mb-1 md:mb-1.5 text-sm md:text-sm text-orange-400">{{article.category}} · ZeroPage</p>
                      <h2 class="mb-1 md:mb-1.5 text-lg md:text-xl font-semibold  text-white transition group-hover:text-orange-300 group-hover:duration-500">{{ article.title }}</h2>
